...
SERVICIOS_FILE = "servicios_predeterminados.json"
...
def app():
    ...
    frame_formulario.grid(row=len(campos), column=1, sticky='w', padx=5, pady=5)

    servicios_text = tk.Text(frame_formulario, width=60, height=6)
    servicios_text.grid(row=1, column=0, columnspan=2, sticky='w')

    servicio_var = tk.StringVar()
    servicio_menu = ttk.Combobox(
        frame_formulario,
        textvariable=servicio_var,
        values=[s for s in servicios_predeterminados if s.strip()],
        state="readonly",
        width=60
    )
    servicio_menu.grid(row=0, column=0, sticky='w')

    def insertar_servicio():
        seleccion = servicio_var.get()
        if seleccion:
            servicios_text.insert(tk.END, seleccion + '\n')

    ttk.Button(frame_formulario, text="Insertar", command=insertar_servicio).grid(row=0, column=1, sticky='w', padx=5)

    def abrir_editor():
        editor = tk.Toplevel(root)
        editor.title("Editar servicios predeterminados")

        entradas = []
        for i in range(5):
            tk.Label(editor, text=f"Servicio {i+1}:").grid(row=i, column=0, sticky='e')
            entrada = tk.Entry(editor, width=80)
            entrada.insert(0, servicios_predeterminados[i] if i < len(servicios_predeterminados) else "")
        ...
        def guardar():
            ...
            nonlocal servicios_predeterminados
            servicios_predeterminados = nuevos
            servicio_menu['values'] = [s for s in nuevos if s.strip()]
            editor.destroy()

        ttk.Button(editor, text="Guardar", command=guardar).grid(row=6, column=1, pady=10, sticky='e')

    ttk.Button(frame_formulario, text="✏️", command=abrir_editor).grid(row=0, column=2, padx=5)
